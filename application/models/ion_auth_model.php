...
class Ion_auth_model extends CI_Model
{
	...
	public function login_groupsales($identity, $password, $remember=FALSE){
		$this->trigger_events('pre_login');
		...
		$sql = 'select ' . $this->identity_column . ', username, email, a.id, password, active, last_login 
			from ' . $this->tables['users'] . '  a , ' . $this->tables['users_groups'] . ' b, ' . $this->tables['groups'] . ' c where a.id = b.user_id and b.group_id = c.id and  c.name = "' . $this->config->item('sales_group', 'ion_auth') . '" and ' . $this->identity_column . ' = ?';

		$query = $this->db->query($sql, $identity);
